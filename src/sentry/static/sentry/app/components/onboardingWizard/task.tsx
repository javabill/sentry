...
function Task({router, task, onSkip, onMarkComplete, forwardedRef, organization}: Props) {
  ...
  const supplement = SupplementComponent && (
    <SupplementComponent task={task} completeTask={() => onMarkComplete(task.task)} />
  );

  const skipAction = task.skippable && (
    <SkipConfirm orgId={organization.slug} onSkip={handleSkip}>
      {({skip}) => (
...
const InProgressIndicator = styled(
  ({user, ...p}: React.HTMLAttributes<HTMLDivElement> & {user?: AvatarUser | null}) => (
    <div {...p}>
      <Tooltip
        disabled={!user}
...
const TaskLockedIndicator = styled(IconLock)``;

TaskLockedIndicator.defaultProps = {
  size: 'xs',
